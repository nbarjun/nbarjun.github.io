...
export function drawThreeGeo({ json, radius, materialOptions }) {
  ...
  function drawLine(x_values, y_values, z_values, materialOptions) {
    ...
    // const color = new THREE.Color(0x00FF00) ?? new THREE.Color(0x00FF00);
    const color = new THREE.Color(materialOptions.color);
    const linewidth = materialOptions.linewidth ?? 1;
    const dashed = materialOptions.dashed ?? false;
    const dashSize = materialOptions.dashSize ?? 0.05; // length of dash
    const gapSize = materialOptions.gapSize ?? 0.05;   // length of gap
